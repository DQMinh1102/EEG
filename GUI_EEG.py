...
class RevWindow(MainWindow):
    ...
    def get_data(self):
        while self.run:
            try:
                a = next(self.csv_reader)
                if a != None:
                    if self.tt_f == 1:
                        self.Fp1[:-1] = self.Fp1[1:]                     
                        self.Fp2[:-1] = self.Fp2[1:]
                        self.fp1.append(int(float(a['Fp1']))) 
                        self.fp2.append(int(float(a['Fp2'])))
                        self.fp1_filted = self.butter_bandpass_filter(self.fp1,  self.cutoff_h, self.cutoff_l, self.fs, self.order)
                        self.fp2_filted = self.butter_bandpass_filter(self.fp2,  self.cutoff_h, self.cutoff_l, self.fs, self.order)
                        self.Fp1[-1] = self.fp1_filted[-1]  
                        self.Fp2[-1] = self.fp2_filted[-1]
                        self.fp1.pop(0)
                        self.fp2.pop(0)
                        ...
                        self.Fp1[:-1] = self.Fp1[1:]                      
                        self.Fp2[:-1] = self.Fp2[1:]
                        self.Fp1[-1] = int(float(a['Fp1'])) - 675   
                        self.Fp2[-1] = int(float(a['Fp2'])) - 675
                    self.ptr += 1
                    time.sleep(0.006)
                else:
                    break
    ...
    def on_combobox_func(self, text):
        ...
        self.ptr = -self.windowWidth
        try:
            self.csv_file = open(text, 'r')
            self.com_lbl.setText("Reviewing file:    " + text)
            self.com_lbl.setStyleSheet( " color: #ff9c75;"
                                        "background-color: #1d1f24;"
                                    "selection-background-color: red;"
                                    "border-radius: 10px;"
                                    "font: bold 17px;"
...
# -----------Finding Port-----------
        ...
        self.port = "COM"
        for self.port, desc, hwid in sorted(self.ports):
            if hwid[78:86] == "31F5AF7F":
                break
        self.timer = QtCore.QTimer()
...
class Controller:
    def __init__(self):
        self.get_port = "Please turn on your bluetooth"
    def show_load(self):
        self.l = SplashScreen()
        self.l.switch_window.connect(self.show_first)
        if self.l.port != "COM":
            self.get_port = self.l.port
        self.l.show()

    def show_first(self):
        self.f = FirstWindow()
        self.f.port_name = self.get_port
        # self.f.stt_port()
        self.f.connect_lb.setText("* Your COM port is:    " + self.f.port_name)
        self.f.switch_window.connect(self.show_main)
        self.f.show()
        
    def show_main(self):
        self.w = MainWindow()        
        # self.w.portName = self.get_port
